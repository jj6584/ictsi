...
<div class="panel panel-primary">
  ...
  <div class="panel-heading">
    <h3 class="panel-title"><font face="bebas" size="8"><center>Manpower Request FORM</center></font></h3>
  </div>
  <div class="panel-body">

<div class="request-form">

<form action = "<?=base_url()?>emp/request_validation" method="post" >
  <div class="form-group <?php if(form_error('position')){echo "has-error";}?>">
    <label for="Position">Vacant Position:</label>
    <input type="text" class="form-control" name="position" id="position" placeholder="Vacant Position"
    value="<?php echo set_value(form_error('position'));?>"
        data-toggle="<?php if(form_error('position')){
          echo "popover";}else{echo "";}?>"
        data-trigger="focus" data-placement="bottom"
        data-html="true" data-content="<?php echo form_error('position');?>">
  </div>
<?php
	$skills = $this->model_emp->getSkills();

?>
  <div class="form-group <?php if(form_error('deptname')){echo "has-error";}?>">
    <label for="deptName">Department Name:</label>
    <input type="text" class="form-control" name="deptname" id="deptname" placeholder="Department Name"
    value="<?php echo set_value(form_error('deptname'));?>"
        data-toggle="<?php if(form_error('deptname')){
          echo "popover";}else{echo "";}?>"
        data-trigger="focus" data-placement="bottom"
        data-html="true" data-content="<?php echo form_error('deptname');?>">
  </div>

  <div class="form-group <?php if(form_error('skills')){echo "has-error";}?>">
    <label for="skills">Skills:</label>

<select class="js-example-basic-multiple" multiple="multiple" id="skills" name="skills[]" width="50%"
...
<div class="form-group <?php if(form_error('year')){echo "has-error";}?>">
  ...
  <div class="form-group <?php if(form_error('jobslots')){echo "has-error";}?>">
    <input type="text" class="form-control" name="jobslots" id="pos" placeholder="Job slot?"
    value="<?php echo set_value(form_error('jobslots'));?>"
        data-toggle="<?php if(form_error('jobslots')){
          echo "popover";}else{echo "";}?>"
        data-trigger="focus" data-placement="bottom"
        data-html="true" data-content="<?php echo form_error('jobslots');?>">
  </div>


  <div class="form-group <?php if(form_error('emptype')){echo "has-error";}?>">
    <label for="skills">Employment Type:</label>
    <select class="form-control" name="emptype" id="emptype"
    value="<?php echo set_value(form_error('emptype'));?>"
        data-toggle="<?php if(form_error('emptype')){
          echo "popover";}else{echo "";}?>"
        data-trigger="focus" data-placement="bottom"
        data-html="true" data-content="<?php echo form_error('emptype');?>">
  ...
  <button class="btn btn-primary btn-block" type="submit"><font face="mohave" size="5">Send Request</button>
</form>
