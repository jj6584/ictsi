...
<nav class="navbar navbar-default">
 ...
 $application = $this->model_users->get_applications();
...
if($key['app_status'] == 0){
        $val = "<span class='label label-default'>UNDER REVIEW</span>";
     }else if($key['app_status'] == 1){
        $val = "<span class='label label-primary'>FOR INTERVIEW</span>";
     } else if($key['app_status'] == 2){
        $val = "<span class='label label-info'>FOR HIRING</span>";
    }else if($key['app_status'] == 3){
        $val = "<span class='label label-success'>HIRED</span>";
     } else if($key['app_status'] == 4){
        $val = "<span class='label label-danger'>NOT QULIFIED</span>";
     }



echo "<tr>";
echo "<td>".anchor('main/job_description/'.$key['job_id'] ,  $key['job_title'])."</td>";
